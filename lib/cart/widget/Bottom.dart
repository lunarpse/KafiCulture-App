...
class Bottom extends ConsumerWidget {
  ...
  final tip;
 final double gst;
  Bottom({super.key,required this.gst, required this.subtotal, required this.tip});
  
  @override
  Widget build(BuildContext context,WidgetRef ref) {
    
   // print("hhh $gst    $tip  ${double.parse(subtotal.toStringAsFixed(2))}");
    double total=double.parse(gst.toStringAsFixed(2))+ double.parse(subtotal.toStringAsFixed(2));
          ...
          children: [
            Row(
              ...
              children: [
                Icon(Icons.receipt),
                SizedBox(width: 10,),
                Text("Bill Details",style: TextStyle(fontWeight: FontWeight.bold,fontSize: 27),),
              ],
            ),
            Charges(
                name: "Subtotal",
            ...
            Charges(
                name: "Total",
                cost: double.parse(total.toStringAsFixed(2))),
            SizedBox(height: 10),
            Row(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                ElevatedButton(
                  ...
                  onPressed: () {
                    ref.watch(TipProvider.notifier).setgst(double.parse(gst.toStringAsFixed(2)));
